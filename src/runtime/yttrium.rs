...
impl Yttrium {
    pub fn new() -> Result<Yttrium, YttriumError> {
        if !YTTRIUM_IN_USE.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).unwrap() {
            Ok(Yttrium { _not_send: PhantomData, panels: Arena::new()})
        } else {
...
impl Yttrium {
    pub(super) fn create_panel(&mut self, panel: Panel) -> Result<generational_arena::Index, YttriumError> {
...
pub struct InterfaceManager<'ytt> {
    yttrium: &'ytt mut super::yttrium::Yttrium
}

impl InterfaceManager {
    pub fn get<I: Interface>() -> Result<I, InterfaceError> {
        Ok(())
    }
}
